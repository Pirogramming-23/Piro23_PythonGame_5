...
"""
    ...
    while True:
        ...
        if choice not in ['1', '2', '3', '4', '5']:
            ...
            continue
        choice = int(choice)
        drink_amount = {1: 2, 2: 4, 3: 6, 4: 8, 5: 10}[choice] # dictionary 매핑 방식
...
def invite_friends(player_name, player_limit):
    ...
    while True:
        try:
            count = int(input("함께 취할 친구들은 얼마나 필요하신가요? (사회적 거리두기로 인해 최대 3명까지 초대할 수 있어요!) : "))
            if 1<=count<=3:
                break
            else:
                print("⚠️ 0명에서 3명 사이로 입력해주세요!\n")
        except ValueError:
            print("⚠️ 숫자를 입력해주세요!\n")
...
""".format(game_list[0][0],game_list[1][0],game_list[2][0],game_list[3][0])
        ...
        if player['name'] == my_name:
            ...
            answer = input("술게임 진행중 ! 다른 사람의 턴입니다. 그만하고 싶으면 'exit'를, 계속하고 싶으면 아무 키나 눌러주세요 ! : ").strip().lower()
            if answer == 'exit':
                print("게임을 종료합니다!")
                exit()
            print(f"{player['name']} (이)가 좋아하는 랜덤 게임 ~ 랜덤 게임 ~ 무슨 게임 ? : ", end="")
